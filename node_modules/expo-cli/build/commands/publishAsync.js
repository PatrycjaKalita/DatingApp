...
async function actionAsync(projectRoot, options = {}) {
  ...
  const owner = (0, _projects().getProjectOwner)(user, exp);
  ...
  if (runtimeVersion) {
    _log().default.log(`\u203A Runtime version: ${_log().default.chalk.bold(runtimeVersion)}`);
  } else if (sdkVersion) {
    _log().default.log(`\u203A Expo SDK: ${_log().default.chalk.bold(sdkVersion)}`);
  }

  _log().default.log(`\u203A Release channel: ${_log().default.chalk.bold(options.releaseChannel)}`);

  _log().default.log(`\u203A Workflow: ${_log().default.chalk.bold(target.replace(/\b\w/g, l => l.toUpperCase()))}`);

  if (user.kind === 'robot') {
    _log().default.log(`\u203A Owner: ${_log().default.chalk.bold(owner)}`);
  }
...
function logManifestUrl({
  ...
  _log().default.log(`📝  Manifest: ${_log().default.chalk.bold(TerminalLink().fallbackToUrl(url, manifestUrl))} ${_log().default.chalk.dim(TerminalLink().learnMore('https://expo.fyi/manifest-url'))}`);
}
/**
 *
