...
async function installCocoaPodsAsync(projectRoot) {
  ...
  if (!(await packageManager.isCLIInstalledAsync())) {
    try {
      ...
      step.stopAndPersist({
        symbol: '⚠️ ',
        text: _log().default.chalk.red('Unable to install the CocoaPods CLI.')
      });
  ...
  try {
    ...
    step.stopAndPersist({
      symbol: '⚠️ ',
      text: _log().default.chalk.red('Something went wrong running `pod install` in the `ios` directory.')
    });
