...
function logNoConfig() {
  _log().default.log(_log().default.chalk.yellow(`No Expo config was found. Please create an Expo config (${_log().default.chalk.bold`app.json`} or ${_log().default.chalk.bold`app.config.js`}) in your project root.`));
}
...
function warnAboutConfigAndThrow(type, message, edits) {
  ...
  if (type === 'warn') {
    ...
    _log().default.log(_log().default.chalk.yellow(message));
  } else {
    logNoConfig();
  }
...
function notifyAboutManualConfigEdits(edits) {
  _log().default.log(_log().default.chalk.cyan(`Please add the following to your Expo config`));
