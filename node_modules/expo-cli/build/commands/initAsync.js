...
async function resolveProjectRootAsync(input) {
  ...
  if (!name) {
    const message = ['', 'Please choose your app name:', `  ${_log().default.chalk.green(`${_commander().default.name()} init`)} ${_log().default.chalk.cyan('<app-name>')}`, '', `Run ${_log().default.chalk.green(`${_commander().default.name()} init --help`)} for more info`, ''].join('\n');
...
async function actionAsync(incomingProjectRoot, command) {
  ...
  if (resolvedTemplate) {
    ...
    templateSpec = (0, _npmPackageArg().default)(`${template}${versionParam}`);
  }
  ...
  const extractTemplateStep = (0, _ora().logNewSection)('Downloading and extracting project files.');
  let projectPath;

  try {
    projectPath = await (0, _extractTemplateAppAsync().extractAndPrepareTemplateAppAsync)(templateSpec, projectRoot, initialConfig);
  ...
  } catch {// todo: check if git is installed, bail out
  }
}
...
async function initGitRepoAsync(root, flags = {
  ...
  try {
    ...
    !flags.silent && _log().default.log('New project is already inside of a git repo, skipping git init.');
  } catch (e) {
    if (e.errno === 'ENOENT') {
  ...
  try {
    ...
    !flags.silent && _log().default.log('Initialized a git repository.');
