...
async function configureManagedProjectAsync({
  ...
  const possibleUsername = process.env.EXPO_CLI_USERNAME || process.env.EAS_BUILD_USERNAME || (await _xdl().UserManager.getCurrentUsernameAsync());
  let {
    exp: config
  } = (0, _prebuildConfig().getPrebuildConfig)(projectRoot, {
    platforms,
    packageName,
    bundleIdentifier,
