...
function _default(program) {
  (0, _applyAsyncAction().applyAsyncAction)(program.command('upgrade [sdk-version]').alias('update').description('Upgrade the project packages and config for the given SDK version').helpGroup('info').option('--npm', 'Use npm to install dependencies. (default when package-lock.json exists)').option('--yarn', 'Use Yarn to install dependencies. (default when yarn.lock exists)'), () => Promise.resolve().then(() => _interopRequireWildcard(require('./upgradeAsync'))));
}
