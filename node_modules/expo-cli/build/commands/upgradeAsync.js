...
async function actionAsync(requestedSdkVersion, options) {
  const {
    projectRoot,
    workflow
...
async function getUpdatedDependenciesAsync(projectRoot, workflow, targetSdkVersion, targetSdkVersionString) {
  ...
  const bundledNativeModules = await (0, _bundledNativeModules().getBundledNativeModulesAsync)(projectRoot, targetSdkVersionString); // Smoosh regular and dev dependencies together for now
  ...
  const removed = []; // Add expo-random as a dependency if using expo-auth-session and upgrading to
  ...
  if (dependencies['@react-native-community/async-storage'] && _semver().default.gte(targetSdkVersionString, '41.0.0')) {
    dependencies['@react-native-async-storage/async-storage'] = bundledNativeModules['@react-native-async-storage/async-storage'];
...
function getDependenciesFromBundledNativeModules({
  ...
  if (sdkVersion && projectDependencies['jest-expo']) {
    ...
    if (targetSdkVersion !== null && targetSdkVersion !== void 0 && targetSdkVersion.beta) {
...
async function makeBreakingChangesToConfigAsync(projectRoot, targetSdkVersionString) {
  ...
  const step = (0, _ora().logNewSection)('Updating your app.json to account for breaking changes (if applicable)...');
  const {
    exp: currentExp,
    dynamicConfigPath,
    staticConfigPath
  ...
  const {
    rootConfig
  } = (0, _config().readConfigJson)(projectRoot);
    ...
    switch (targetSdkVersionString) {
      ...
      case '37.0.0':
        if ((rootConfig === null || rootConfig === void 0 ? void 0 : (_rootConfig$expo = rootConfig.expo) === null || _rootConfig$expo === void 0 ? void 0 : (_rootConfig$expo$andr = _rootConfig$expo.androidNavigationBar) === null || _rootConfig$expo$andr === void 0 ? void 0 : _rootConfig$expo$andr.visible) !== undefined) {
          ...
          step.stopAndPersist({
            symbol: '⚠️ ',
            text: _chalk().default.red(`Please manually update "androidNavigationBar.visible" according to ${(0, _terminalLink().default)('this documentation', 'https://docs.expo.dev/versions/latest/config/app/#androidnavigationbar')}`)
          });
        } else {
          step.succeed('No additional changes necessary to app.json config.');
        }
...
async function shouldBailWhenUsingLatest(currentSdkVersionString, targetSdkVersionString) {
  ...
  if (targetSdkVersionString === currentSdkVersionString) {
    ...
    if (!answer) {
      _log().default.log('Follow the Expo blog at https://blog.expo.dev for new release information!');
...
async function upgradeAsync({
  ...
  if (targetSdkVersionString !== currentSdkVersionString) {
    const installingPackageStep = (0, _ora().logNewSection)(`Installing the ${expoPackageToInstall} package...`);
  ...
  const removingSdkVersionStep = (0, _ora().logNewSection)('Validating configuration.');
  ...
  const updatingPackagesStep = (0, _ora().logNewSection)('Updating packages to compatible versions (where known).');
  ...
  const {
    ...
    removed
  } = await getUpdatedDependenciesAsync(projectRoot, workflow, targetSdkVersion, targetSdkVersionString); // Split updated packages by dependencies and devDependencies
  ...
  const clearingCacheStep = (0, _ora().logNewSection)('Clearing the packager cache.');
  ...
  if (removed.length) {
    ...
    _log().default.log(_chalk().default.bold(`🗑️ The following packages were removed:`));

    _log().default.log(_chalk().default.grey.bold(removed.join(', ')));
  ...
  if (removed.includes('@react-native-community/async-storage')) {
    ...
    _log().default.log(_chalk().default.bold(`🚨 @react-native-community/async-storage has been renamed to @react-native-async-storage/async-storage. The dependency has been updated automatically, you will now need to either update the imports in your source code manually, or run \`npx expo-codemod sdk41-async-storage './**/*'\`.`));
  ...
  if (targetSdkVersion && targetSdkVersion.releaseNoteUrl) {
    ...
    if (_getenv().default.boolish('EXPO_BETA', false)) {
      ...
      _log().default.gray(`Unable to find release notes for ${targetSdkVersionString}, please try to find them on https://blog.expo.dev to learn more about other potentially important upgrade steps and breaking changes.`);
    }
  }
...
async function maybeCleanNpmStateAsync(packageManager) {
  ...
  if (packageManager instanceof PackageManager().NpmPackageManager) {
    const cleaningNpmStateStep = (0, _ora().logNewSection)('Removing package-lock.json and deleting node_modules.');
    ...
    if (shouldInstallNodeModules) {
      const reinstallingNodeModulesStep = (0, _ora().logNewSection)('Installing node_modules and rebuilding package-lock.json.');
...
//# sourceMappingURL=upgradeAsync.js.map