...
const helpGroupOrder = ['core', 'auth', 'client', 'info', 'publish', 'build', 'credentials', 'notifications', 'url', 'webhooks', 'upload', 'eject', 'experimental', 'internal'];
    ...
    try {
      ...
      if (err instanceof _CommandError().AbortCommandError || err instanceof _CommandError().SilentError) {// Do nothing when a prompt is cancelled or the error is logged in a pretty way.
        ...
        if (err.code === 'EJSONEMPTY') {
          ...
          const {
            formatStackTrace
          } = await Promise.resolve().then(() => _interopRequireWildcard(require('./utils/formatStackTrace')));
  ...
  return this.asyncAction(async (projectRoot, ...args) => {
    ...
    if (opts.config) {
      ...
      if (opts.config === true) {
        ...
        _log().default.log(_log().default.chalk.green(`  expo ${this.name()} --config ${_log().default.chalk.cyan(`<app-config>`)}`));
...
async function bootstrapAnalyticsAsync() {
  if (_xdl().Env.shouldDisableAnalytics()) {
    return; // do not allow E2E to fire events
  }

  _xdl().Analytics.initializeClient('vGu92cdmVaggGA26s3lBX6Y5fILm8SQ7', {
    apiKey: '1wHTzmVgmZvNjCalKL45chlc2VN',
    dataPlaneUrl: 'https://cdp.expo.dev'
  }, packageJSON.version);

  _xdl().UnifiedAnalytics.initializeClient('u4e9dmCiNpwIZTXuyZPOJE7KjCMowdx5', {
  ...
  const stream = {
    ...
    stream: {
      write: chunk => {
        if (chunk.code) {
          switch (chunk.code) {
            case _xdl().NotificationCode.START_PROGRESS_BAR:
              {
            ...
            case _xdl().NotificationCode.TICK_PROGRESS_BAR:
              {
            ...
            case _xdl().NotificationCode.STOP_PROGRESS_BAR:
              {
                const bar = _log().default.getProgress();
