...
async function ensureProjectExistsAsync(user, {
  ...
  const spinner = (0, _ora().ora)(`Ensuring project ${_log().default.chalk.bold(projectFullName)} is registered on Expo servers`).start();
...
function getProjectOwner(user, exp) {
  if (user.kind === 'robot' && !exp.owner) {
    throw new (_CommandError().default)('ROBOT_OWNER_ERROR', 'The "owner" manifest property is required when using robot users. See: https://docs.expo.dev/versions/latest/config/app/#owner');
  }
