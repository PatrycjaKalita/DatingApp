...
async function _maybeRunModifiedExpoUpdatesPluginAsync(config) {
  ...
  if (_fsExtra().default.existsSync(expoPlistPath)) {
    ...
    expoPlistForProject = _configPlugins().IOSConfig.Updates.setUpdatesConfig(exp, expoPlistForProject,
    /*expoUsername*/
  ...
  if (_fsExtra().default.existsSync(androidManifestXmlPath)) {
    var _currentlyConfiguredA, _currentConfiguredMan, _currentConfiguredMan2, _currentlyConfiguredM;
    ...
    const [currentConfiguredManifestApplication] = (_currentlyConfiguredA = currentlyConfiguredAndroidManifest.manifest.application) !== null && _currentlyConfiguredA !== void 0 ? _currentlyConfiguredA : [];
    ...
    const inferredAndroidManifest = _configPlugins().AndroidConfig.Updates.setUpdatesConfig(exp, currentlyConfiguredAndroidManifest,
    /*username*/
    null);
    ...
    if (currentlyConfiguredAndroidUpdateURL === PLACEHOLDER_URL) {
      ...
      var _inferredManifestAppl;

      // Log warnings if this is not the first publish and critical properties seem misconfigured
      const [inferredManifestApplication] = inferredAndroidManifest.manifest.application;
      const inferredMetaDataAttributes = (_inferredManifestAppl = inferredManifestApplication['meta-data']) === null || _inferredManifestAppl === void 0 ? void 0 : _inferredManifestAppl.map(md => md['$']);
