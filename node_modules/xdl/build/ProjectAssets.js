...
exports.resolveAndCollectExpoUpdatesManifestAssets = resolveAndCollectExpoUpdatesManifestAssets;
...
async function resolveExpoUpdatesManifestAssets({
  ...
  const assetInfos = await Promise.all(assetSchemas.map(async manifestField => {
    ...
    if (/^https?:\/\//.test(pathOrURL)) {
      ...
      _internal().ProjectUtils.logError(projectRoot, 'expo', `Unable to resolve asset "${pathOrURL}" from "${manifestField}" in your app.json or app.config.js`);
  ...
  assetSchemas.forEach((manifestField, index) => (0, _set().default)(manifest, `${manifestField}Asset`, assetInfos[index]));
}
...
async function resolveAndCollectExpoUpdatesManifestAssets(projectRoot, exp, urlResolver) {
  const manifestAssets = [];
  await resolveExpoUpdatesManifestAssets({
    ...
    async assetKeyResolver(assetPath) {
      ...
      const contents = await _fsExtra().default.readFile(absolutePath); // Expo Updates spec dictates that this hash is sha256

      const hash = _crypto().default.createHash('sha256').update(contents).digest('hex');

      manifestAssets.push({
        url: urlResolver(assetPath),
        hash,
        key: assetPath,
        contentType: (_mime$getType = _mime().default.getType(absolutePath)) !== null && _mime$getType !== void 0 ? _mime$getType : 'application/octet-stream'
      });
      return assetPath;
    }
