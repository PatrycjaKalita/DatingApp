...
async function installExpoAsync({
  ...
  _internal().Logger.notifications.info({
    code: _internal().NotificationCode.STOP_LOADING
  });
  ...
  try {
    ...
    if (devClient) {
      ...
      if (isManaged) {
        ...
        applicationId = await _configPlugins().AndroidConfig.Package.getApplicationIdAsync(projectRoot);
...
async function openProjectAsync({
  ...
  try {
    ...
    const projectUrl = await _internal().UrlUtils.constructDeepLinkAsync(projectRoot, {
      scheme
    });
    const {
      exp
    ...
    if (e.isAbortError) {
      ...
      _internal().Logger.global.error(`Couldn't start project on Android: ${e.message}`);
    }
...
async function openWebProjectAsync({
  ...
  try {
    ...
    _internal().Logger.global.error(`Couldn't open the web project on Android: ${e.message}`);
...
async function checkSplashScreenImages(projectRoot) {
  var _ref, _exp$android$splash$r, _exp$android2, _exp$android2$splash, _exp$splash, _exp$splash2, _exp$android3;
  ...
  const splashScreenMode = (_ref = (_exp$android$splash$r = (_exp$android2 = exp.android) === null || _exp$android2 === void 0 ? void 0 : (_exp$android2$splash = _exp$android2.splash) === null || _exp$android2$splash === void 0 ? void 0 : _exp$android2$splash.resizeMode) !== null && _exp$android$splash$r !== void 0 ? _exp$android$splash$r : (_exp$splash = exp.splash) === null || _exp$splash === void 0 ? void 0 : _exp$splash.resizeMode) !== null && _ref !== void 0 ? _ref : 'contain'; // only mode `native` is handled by this check
  ...
  const androidSplash = (_exp$android3 = exp.android) === null || _exp$android3 === void 0 ? void 0 : _exp$android3.splash;
...
function getAPKDirectory(projectRoot) {
  return _path().default.join(projectRoot, 'android', 'app', 'build', 'outputs', 'apk');
}
