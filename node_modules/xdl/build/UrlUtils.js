...
async function constructDeepLinkAsync(projectRoot, opts, requestHostname) {
  const {
    devClient
  } = await _internal().ProjectSettings.getPackagerOptsAsync(projectRoot);
...
function assertValidOptions(opts) {
  ...
  if (error) {
    throw new (_internal().XDLError)('INVALID_OPTIONS', error.toString());
  }
...
async function ensureOptionsAsync(projectRoot, opts) {
  ...
  const defaultOpts = await _internal().ProjectSettings.getPackagerOptsAsync(projectRoot);
