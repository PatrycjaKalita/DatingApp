...
async function publishAsync(projectRoot, options = {}) {
  var _options$target, _bundles$android$herm, _bundles$ios$hermesBy, _bundles$android$herm2, _bundles$ios$hermesSo, _exp$ios, _exp$android, _options$releaseChann;
  ...
  const androidSourceMap = hasHooks ? (_bundles$android$herm2 = bundles.android.hermesSourcemap) !== null && _bundles$android$herm2 !== void 0 ? _bundles$android$herm2 : bundles.android.map : null;
  const iosSourceMap = hasHooks ? (_bundles$ios$hermesSo = bundles.ios.hermesSourcemap) !== null && _bundles$ios$hermesSo !== void 0 ? _bundles$ios$hermesSo : bundles.ios.map : null;
  let response;
  ...
  try {
    ...
    _internal().Sentry.captureException(e);
  ...
  const url = options.releaseChannel && options.releaseChannel !== 'default' ? `${response.url}?release-channel=${options.releaseChannel}` : response.url; // Create project page URL

  const projectPageUrl = response.projectPageUrl ? options.releaseChannel && options.releaseChannel !== 'default' ? `${response.projectPageUrl}?release-channel=${options.releaseChannel}` : response.projectPageUrl : null;
  return { ...response,
    url,
    projectPageUrl
  };
