...
const SHOULD_CLEAR_CONSOLE = _internal().WebpackEnvironment.shouldWebpackClearLogs();

function log(projectRoot, message, showInDevtools = true) {
  if (showInDevtools) {
    _internal().ProjectUtils.logInfo(projectRoot, CONSOLE_TAG, message);
  } else {
    console.log(message);
  }
}
...
function logWarning(projectRoot, message) {
  _internal().ProjectUtils.logWarning(projectRoot, CONSOLE_TAG, message);
}

function logError(projectRoot, message) {
  _internal().ProjectUtils.logError(projectRoot, CONSOLE_TAG, message);
}
...
function createWebpackCompiler({
  ...
  compiler.hooks.done.tap('done', async stats => {
    ...
    if (isSuccessful) {
      _internal().WebpackEnvironment.logEnvironmentInfo(projectRoot, CONSOLE_TAG, config);
    }
