...
class UserManagerInstance {
  ...
  async _getProfileAsync({
    ...
    if ((!this._currentUser || this._currentUser.userId !== user.userId) && user.username && user.username !== '') {
      ...
      _internal().UnifiedAnalytics.identifyUser(user.userId, // userId is used as the identifier in the other codebases (www/website) running unified analytics so we want to keep using it on the cli as well to avoid double counting users
      {
        userId: user.userId,
        currentConnection: user.currentConnection,
        username: user.username,
      ...
      _internal().Analytics.identifyUser(user.username, {
        userId: user.userId,
        currentConnection: user.currentConnection,
        username: user.username,
        userType: user.kind,
