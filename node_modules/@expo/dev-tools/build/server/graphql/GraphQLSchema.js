...
const mergeAsyncIterators_1 = __importDefault(require("../asynciterators/mergeAsyncIterators"));
// for prettier
...
const messageResolvers = {
    level(message) {
        if (message.level <= xdl_1.Logger.DEBUG)
            return 'DEBUG';
        if (message.level <= xdl_1.Logger.INFO)
            return 'INFO';
        if (message.level <= xdl_1.Logger.WARN)
            return 'WARN';
        return 'ERROR';
    },
};
...
const resolvers = {
    ...
    Project: {
        ...
        async manifestUrl(project) {
            if ((await xdl_1.ProjectSettings.getCurrentStatusAsync(project.projectDir)) === 'running') {
                return xdl_1.UrlUtils.constructDeepLinkAsync(project.projectDir);
            }
            else {
                return null;
            }
        ...
        settings(project) {
            return xdl_1.ProjectSettings.readAsync(project.projectDir);
        },
        async config(project) {
            try {
                const { exp } = config_1.getConfig(project.projectDir);
                return exp;
            }
            catch (error) {
                xdl_1.ProjectUtils.logError(project.projectDir, 'expo', error.message);
                return null;
            }
        },
    ...
    ProjectSettings: {
        hostType(projectSettings) {
            if (xdl_1.ConnectionStatus.isOffline() && projectSettings.hostType === 'tunnel') {
                return 'lan';
            }
            else {
    ...
    Query: {
        ...
        userSettings() {
            return xdl_1.UserSettings.readAsync();
        },
        ...
        async processInfo(parent, args, context) {
            return {
                isAndroidSimulatorSupported: xdl_1.Android.isPlatformSupported(),
                isIosSimulatorSupported: xdl_1.Simulator.isPlatformSupported(),
            };
        },
        async user() {
            const username = await xdl_1.UserManager.getCurrentUsernameAsync();
            return { username };
        },
    },
    Mutation: {
        async openWeb(parent, props, context) {
            ...
            try {
                await xdl_1.Webpack.openAsync(currentProject.projectDir);
                return {
                    success: true,
                    error: null,
                };
        ...
        async openSimulator(parent, { platform }, context) {
            ...
            let result;
            if (platform === 'ANDROID') {
                result = await xdl_1.Android.openProjectAsync({
                    projectRoot: currentProject.projectDir,
                    shouldPrompt: false,
                });
            }
            else {
                result = await xdl_1.Simulator.openProjectAsync({
                    projectRoot: currentProject.projectDir,
                    shouldPrompt: false,
                });
            }
        ...
        async publishProject(parent, { releaseChannel }, context) {
            ...
            try {
                const result = await xdl_1.Project.publishAsync(projectDir, { releaseChannel });
                return result;
            }
            catch (error) {
                xdl_1.ProjectUtils.logError(projectDir, 'expo', error.message);
                throw error;
            }
        },
        async setProjectSettings(parent, { settings }, context) {
            const currentProject = context.getCurrentProject();
            const previousSettings = await xdl_1.ProjectSettings.readAsync(currentProject.projectDir);
            const updatedSettings = await xdl_1.ProjectSettings.setAsync(currentProject.projectDir, settings);
            ...
            if (previousSettings.hostType !== 'tunnel' && updatedSettings.hostType === 'tunnel') {
                try {
                    await xdl_1.Project.startTunnelsAsync(currentProject.projectDir, { autoInstall: true });
                }
                catch (e) {
                    xdl_1.ProjectUtils.logWarning(currentProject.projectDir, 'expo', `Error starting tunnel ${e.message}`);
                }
            }
            return {
                ...currentProject,
        ...
        async optimizeAssets(parent, { settings }, context) {
            const currentProject = context.getCurrentProject();
            await spawn_async_1.default('npx', ['expo-optimize'], {
        ...
        async setProjectConfig(parent, { input }, context) {
            ...
            const { exp } = await config_1.writeConfigJsonAsync(currentProject.projectDir, filteredInput);
            return {
                ...currentProject,
                config: exp,
            };
        ...
        async sendProjectUrl(parent, { recipient }, context) {
            const currentProject = context.getCurrentProject();
            const url = await xdl_1.UrlUtils.constructManifestUrlAsync(currentProject.projectDir);
            const result = await xdl_1.Exp.sendAsync(recipient, url);
            await xdl_1.UserSettings.setAsync('sendTo', recipient);
    ...
    Subscription: {
        messages: {
            subscribe(parent, { after }, context) {
                ...
                const iterator = mergeAsyncIterators_1.default(issueIterator, messageIterator);
                return {
                    async next() {
