...
async function default_1(env, argv = {}) {
    ...
    if (isNative) {
        const reactNativeModulePath = resolve_from_1.default.silent(env.projectRoot, 'react-native');
        if (reactNativeModulePath) {
            for (const polyfill of [
                ...
                const resolvedPolyfill = resolve_from_1.default.silent(env.projectRoot, `react-native/Libraries/${polyfill}`);
                if (resolvedPolyfill)
                    appEntry.unshift(resolvedPolyfill);
            }
        }
    }
    else {
    ...
    let webpackConfig = {
        ...
        plugins: [
            ...
            !isCI && new plugins_1.ExpoProgressBarPlugin(),
        ].filter(Boolean),
        module: {
