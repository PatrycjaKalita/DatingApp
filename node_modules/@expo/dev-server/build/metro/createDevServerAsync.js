...
async function getConfig(projectRoot, config) {
    const { getDefaultConfig, mergeConfig } = importMetroFromProject_1.importMetroConfigFromProject(projectRoot);
...
async function runMetro(projectRoot, config) {
    const MetroServer = importMetroFromProject_1.importMetroServerFromProject(projectRoot);
...
async function createDevServerAsync(projectRoot, { config, logger, onReady }) {
    const serverApp = connect_1.default();
    const metroServer = await runMetro(projectRoot, config);
    const { InspectorProxy } = importMetroFromProject_1.importInspectorProxyServerFromProject(projectRoot);
    const MetroHmrServer = importMetroFromProject_1.importMetroHmrServerFromProject(projectRoot);
    const attachWebsocketServer = importMetroFromProject_1.importMetroLibAttachWebsocketServerFromProject(projectRoot);
    ...
    const httpServer = http_1.default.createServer(serverApp);
    ...
    return new Promise((resolve, reject) => {
        httpServer.listen(config.server.port, () => {
            ...
            const websocketServer = new MetroHmrServer(metroServer.getBundler(), metroServer.getCreateModuleId(), config);
            // attachHmrServer
...
//# sourceMappingURL=createDevServerAsync.js.map